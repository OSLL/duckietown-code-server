...
- no modal dialog is show when the ESLint extension tries to load a ESLint library for the first time and an approval is necessary. Instead the ESLint status bar item changes to ![ESLint status icon](https://github.com/Microsoft/vscode-eslint/raw/master/images/2_1_10/eslint-status.png) indicating that the execution is currently block.
