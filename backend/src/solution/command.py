...
class DTCommand(DTCommandAbs):
    ...
    def command(shell, args: list):
        ...
        parser.add_argument(
            "--loop",
            default=False,
            action="store_true", help="(Experimental) Whether to run the LOOP image"
        )
        parser.add_argument(
            "-A",
            "--argument",
        ...
        # ---
        dtslogger.info("Project workspace: %s", parsed.workdir)
        ...
        if parsed.arch is None:
            ...
            dtslogger.info("Target architecture automatically set to %s.", parsed.arch)
        ...
        for mount_point in DEFAULT_MOUNTS:
            if parsed.machine == DEFAULT_MACHINE:
                # we are running locally, check if the mount_point exists
                if not os.path.exists(mount_point):
                    dtslogger.warning(
                        "The mount_point '%s' does not exist. This can create issues inside the container.", mount_point
                    )
                    continue
            mount_option += ["-v", "{:s}:{:s}".format(mount_point, mount_point)]
        ...
        e_point = json.loads(e_point)
        if "ServerErrors" in e_point:
            dtslogger.error("\n".join(e_point["ServerErrors"]))
            return
        e_point["MemTotal"] = human_size(e_point["MemTotal"])
        print(DOCKER_INFO.format(**e_point))
        # print info about multiarch
        msg = "Running an image for {} on {}.".format(parsed.arch, e_point["Architecture"])
        dtslogger.info(msg)
        # register bin_fmt in the target machine (if needed)
        if not parsed.no_multiarch:
            compatible_archs = BUILD_COMPATIBILITY_MAP[CANONICAL_ARCH[e_point["Architecture"]]]
            if parsed.arch not in compatible_archs:
                ...
                msg = "Running an image for {} on {}. Multiarch not needed!".format(
                    parsed.arch, e_point["Architecture"]
                )
                dtslogger.info(msg)
        ...
        if parsed.sync:
            ...
            dtslogger.info("Syncing code with %s...", parsed.machine.replace('.local', ''))
            ...
            for project_path in projects_to_sync:
                cmd = f'rsync --rsh="sshpass -p quackquack ssh -o StrictHostKeyChecking=no -l duckie" --chown=duckie:duckie --archive {project_path} {remote_path}'
